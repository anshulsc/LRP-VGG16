...
def apply_lrp_on_vgg16(model, image):
    ...
    #print("Extracted layers in step 1 are",layers)
    ...
    activations.append(torch.FloatTensor([one_hot_output]).to(device))
    ...
    for layer in range(0, n_layers)[::-1]:
        ...
        if isinstance(current, torch.nn.Conv2d) or \
           ...
           isinstance(current, torch.nn.Linear):
            ...
            #the composite LRP approach uses the epsilon-rule, z-rule, alpha-beta rule, and the flat-rule to calculate the positive and negative relevance scores.
            #In composite LRP, the relevance scores are decomposed into two components: the positive and negative relevance scores.
            #The advantage of using composite LRP is that it can provide a more accurate and fine-grained explanation of the predictions made by deep neural networks.
...
# %%
#Another cariant to improve the interpretability of the lrp function.
#In the zB-rule, the relevance scores are propagated from the output layer to the input layer of the network using a backward pass that involves a sequence of "zB-layers." A zB-layer is a layer that applies a modified version of the epsilon-rule, which uses a "zB-factor" to adjust the relevance scores. The zB-factor is a scaling factor that depends on the activation function used in the layer, and it is designed to compensate for the piecewise linear nature of the function.
#the zB-rule is a variant of LRP that is designed to improve the interpretability of neural networks with piecewise linear activation functions, by providing more accurate and meaningful relevance scores for the input features.