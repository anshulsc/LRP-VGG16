...
class CNNModel(nn.Module):
    def __init__(self):
        ...
        self.efficientnet = models.efficientnet_b0(weights=True)

        # Replace output layer according to our problem
        in_feats = self.efficientnet.classifier.in_features
        self.efficientnet.classifier = nn.Linear(in_feats, 4)
...
# %% Layerwise relevance propagation for EfficientNet
...
def get_linear_layer_indices(model):
    offset = len(model._modules['blocks']) + 1
    indices = []
...
def apply_lrp_on_efficientnet(model, image):
    image = torch.unsqueeze(image, 0)
    # >>> Step 1: Extract layers
    layers = list(model._modules['blocks']) \
                + [model._modules['avg_pool']] \
    ...
    for layer in range(n_layers):
        if layer in linear_layer_indices:
            if layer == 32:
                activations[layer] = activations[layer].reshape((1, 1280, 1, 1))
    ...
    for layer in range(0, n_layers)[::-1]:
        ...
        if isinstance(current, torch.nn.Conv2d) or \
            ...
            # Compute relevance scores for each layer
 ...
 #If the current layer is not the output layer, the relevance values are instead assigned to the relevance values of the next layer, which has already been computed using the same method.
...
image_id = 24
inputs = inputs.to(device)
image_relevances = apply_lrp_on_efficientnet(model, inputs[image_id])
